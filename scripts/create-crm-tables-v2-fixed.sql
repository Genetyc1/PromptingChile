...
-- Solo insertar datos de ejemplo si existen usuarios
DO $$
DECLARE
  user_id UUID;
  deal_id_1 UUID;
...
BEGIN
  ...
  IF user_id IS NOT NULL THEN
    ...
    -- Obtener IDs de los deals creados
    SELECT id INTO deal_id_1 FROM deals WHERE title = 'Implementación CRM Empresa ABC' LIMIT 1;
    SELECT id INTO deal_id_2 FROM deals WHERE title = 'Automatización de Procesos XYZ' LIMIT 1;

    -- Insertar actividades de ejemplo solo si los deals existen
    ...
    RAISE NOTICE 'Datos de ejemplo insertados correctamente con usuario: %', user_id;
  ELSE
    RAISE NOTICE 'No se encontraron usuarios. Los datos de ejemplo no se insertaron.';
  END IF;
END $$;
